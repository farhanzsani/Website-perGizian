...
<div class="space-y-6">
    ...
    <!-- Header Section -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 transition-all duration-300 hover:shadow-md hover:border-emerald-100">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold text-charcoal tracking-tight">Manajemen Pengguna</h2>
                <p class="text-slate text-base mt-1">Kelola semua pengguna aplikasi CarePlate dengan mudah.</p>
            </div>
            <button onclick="openModal()" class="group inline-flex items-center gap-2 bg-leaf hover:bg-emerald-600 text-white px-6 py-3 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md font-semibold text-sm">
                <svg class="h-5 w-5 text-white transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <!-- Total Users Card -->
        <div class="group bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-5 transition-all duration-300 hover:border-emerald-200 hover:shadow-md hover:shadow-emerald-50">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-50 text-leaf group-hover:scale-105 transition-transform duration-300">
                <svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
            </div>
            <div>
                <p class="text-3xl font-extrabold text-charcoal tracking-tight" id="totalUsers">-</p>
                <p class="text-sm font-medium text-slate mt-0.5">Total Pengguna</p>
            </div>
        </div>

        <!-- Admin Card -->
        <div class="group bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-5 transition-all duration-300 hover:border-amber-400/50 hover:shadow-sm">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-50 text-amber-500 group-hover:scale-105 transition-transform duration-300">
                <svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
            </div>
            <div>
                <p class="text-3xl font-extrabold text-charcoal tracking-tight" id="totalAdmins">-</p>
                <p class="text-sm font-medium text-slate mt-0.5">Administrator</p>
            </div>
        </div>

        <!-- User Biasa Card -->
        <div class="group bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-5 transition-all duration-300 hover:border-blue-400/50 hover:shadow-sm">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-50 text-blue-500 group-hover:scale-105 transition-transform duration-300">
                <svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
            </div>
            <div>
                <p class="text-3xl font-extrabold text-charcoal tracking-tight" id="totalRegularUsers">-</p>
                <p class="text-sm font-medium text-slate mt-0.5">User Biasa</p>
            </div>
        </div>
    </div>

    <!-- Table Card -->
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-md hover:border-emerald-100">
        <!-- Table Header -->
        <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4 flex-1">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-charcoal whitespace-nowrap">Daftar Pengguna</span>
                </div>
                ...
                <div class="relative w-full max-w-md">
                    ...
                    <input type="text" id="searchInput" onkeyup="debounceSearch()" 
                        class="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-xl leading-5 bg-gray-50 text-charcoal placeholder-slate focus:outline-none focus:bg-white focus:ring-1 focus:ring-leaf focus:border-leaf sm:text-sm transition-colors" 
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <span class="text-sm text-slate">Tampilkan:</span>
                <div class="relative" x-data="{ 
                    open: false, 
                    selected: 10,
                    select(val) {
                        this.selected = val;
                        this.open = false;
                        document.getElementById('limitSelect').value = val;
                        changeLimit(); // Trigger refresh
                    }
                }">
                    <!-- Hidden Select for JS functionality -->
                    <select id="limitSelect" class="hidden">
                    ...
                    <button @click="open = !open" @click.outside="open = false" 
                        class="flex items-center gap-2 pl-3 pr-2 py-1.5 bg-white border border-gray-200 rounded-lg text-sm text-charcoal hover:border-leaf hover:ring-2 hover:ring-leaf/20 transition-all shadow-sm min-w-[70px] justify-between group">
                    ...
                    <div x-show="open" 
                        ...
                        x-transition:leave-start="opacity-100 translate-y-0"
                        x-transition:leave-end="opacity-0 translate-y-2"
                        class="absolute bottom-full mb-2 w-full bg-white rounded-lg shadow-xl border border-gray-100 z-30 overflow-hidden ring-1 ring-black ring-opacity-5"
                        style="display: none;">
                        <div class="py-1">
                            <button @click="select(10)" class="flex items-center justify-between w-full px-3 py-2 text-sm text-left hover:bg-mint/30 transition-colors" :class="selected == 10 ? 'text-leaf bg-mint/10 font-medium' : 'text-slate'">
                                10
                                <span x-show="selected == 10" class="text-leaf">•</span>
                            </button>
                            <button @click="select(25)" class="flex items-center justify-between w-full px-3 py-2 text-sm text-left hover:bg-mint/30 transition-colors" :class="selected == 25 ? 'text-leaf bg-mint/10 font-medium' : 'text-slate'">
                                25
                                <span x-show="selected == 25" class="text-leaf">•</span>
                            </button>
                            <button @click="select(50)" class="flex items-center justify-between w-full px-3 py-2 text-sm text-left hover:bg-mint/30 transition-colors" :class="selected == 50 ? 'text-leaf bg-mint/10 font-medium' : 'text-slate'">
                                50
                                <span x-show="selected == 50" class="text-leaf">•</span>
                            </button>
                            <button @click="select(100)" class="flex items-center justify-between w-full px-3 py-2 text-sm text-left hover:bg-mint/30 transition-colors" :class="selected == 100 ? 'text-leaf bg-mint/10 font-medium' : 'text-slate'">
                                100
                                <span x-show="selected == 100" class="text-leaf">•</span>
                            </button>
                        </div>
                    </div>
                </div>
                <span class="text-sm text-slate" id="paginationInfo">Menampilkan - dari - data</span>
            </div>
...
<script>
    ...
    function fetchUsers(page = 1) {
        currentPage = page;
        console.log('Lagi ngambil data user nih...', { page });
        
        // Loading effect hanya jika ganti halaman atau filter, bukan refresh background
        // Tapi buat awal kita loading jg gpp di table body
        tableBody.innerHTML = `
            <tr>
                <td colspan="5" class="py-12 text-center">
                    <div class="flex flex-col items-center gap-3">
                        ...
                        <span class="text-slate text-sm">Wait ya, lagi muat data...</span>
                    </div>
                </td>
            </tr>
        `;
        ...
        axios.get("{{ route('admin.users.index') }}", {
            ...
            .then(response => {
                ...
                const data = result.pagination; // Paginator Object from 'pagination' key
                const stats = result.stats;     // Stats Object from 'stats' key
                ...
                if (stats) {
                    ...
                    document.getElementById('totalRegularUsers').textContent = stats.total_regular_users;
                }
                        ...
                        <tr>
                            <td colspan="5" class="py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-charcoal font-medium">Ga ketemu nih user nya</p>
                                    <p class="text-slate text-sm">Coba cari kata kunci lain atau reset filter</p>
                                </div>
                            </td>
                        </tr>
                    `;
    ...
    function debounceSearch() {
        ...
        searchTimeout = setTimeout(() => {
            fetchUsers(1); // Reset ke halaman 1 kalo search
        }, 500);
    }
    
    // Change Limit
    function changeLimit() {
        fetchUsers(1); // Reset ke halaman 1 kalo limit berubah
    }
    ...
    function renderPagination(data) {
        ...
        if (data.prev_page_url) {
            buttons += `<button onclick="fetchUsers(${data.current_page - 1})" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-slate hover:bg-gray-50 hover:text-charcoal transition">Prev</button>`;
        } else {
            buttons += `<button disabled class="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-300 cursor-not-allowed">Prev</button>`;
        }
        
        // Next Button
        if (data.next_page_url) {
            buttons += `<button onclick="fetchUsers(${data.current_page + 1})" class="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-slate hover:bg-gray-50 hover:text-charcoal transition">Next</button>`;
        } else {
            buttons += `<button disabled class="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-300 cursor-not-allowed">Next</button>`;
        }
